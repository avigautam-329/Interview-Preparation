...
void preorder(struct MinHeapNode* root, string res, unordered_map<string, char> &encoded){
    if(root == NULL){
        return;
    }
    
    if(root->data != '$'){
        encoded[res] = root->data;
    }
    
    preorder(root->left , res + '0', encoded);
    preorder(root->right, res + '1', encoded);
...
string decodeHuffmanData(struct MinHeapNode* root, string binaryString)
{
    unordered_map<string  , char> encoded;
    ...
    while(i < binaryString.size()){
        ...
        if(encoded.find(temp) != encoded.end()){
            res += encoded[temp];
            temp = "";
        }
