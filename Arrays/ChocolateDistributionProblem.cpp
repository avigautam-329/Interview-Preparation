...
// Again as we need to minimize the packet numbers.
//		1. First we will sort the array.
//		2. Since we need to distribute between M kids, soo we can take a fixed sized sliding window.
//		3. For a particular sliding window, we have to find the difference of arr[start] and arr[end] and we have to minimize this by moving the sliding windows by one everytime.
...
class Solution{
    ...
    //code
        ...
        while(end < n){
            long long tempDiff =  a[end++] - a[start++];
